...
def item_block(f):
    ...
    # Одна строка живёт сразу в трёх локалях, и меняется во всех трёх разом.
    # Пока в заголовке стояла метка одного языка, правка читалась как работа
    # только по нему — и одно и то же место попадало в список дважды.
    langs = ""
    if f.get("found_in") and all(f.get(l) for l in ("pl", "ua", "en")):
        langs = (f'<p class="fld"><b>Языки</b> Меняется во всех трёх — готовый '
                 f'текст на каждый ниже. Ошибку видно в '
                 f'{esc(f["found_in"])}.</p>')
    ver = ""
